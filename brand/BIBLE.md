...
| 2026-09-20 | Outline mark set v4 (6) — "drawn, not diagrammed": v3's straight scaffolding recut with the tree's organic curves (bark-bowed trunks, curved roofline, hand-drawn tank, tapered neck) after Damian flagged the family not rhyming. Rhyme sheet: refs/outline-family-v4.png |
| 2026-09-20 | Outline mark set v3 (6) | brand/assets/marks/mark-*.svg (outline) + mark-*-solid.svg (kept) + M.* components in app.jsx (source of truth) | Handmade SVG. Tree crown reuses the proven icon-leaf path, outlined at 2.2. Verified: family strip 96/48/24 dark+cream, babel compile + eval of app.jsx. Two recut loops (tree starburst ×2, jug ear). |
| 2026-09-21 | og-image "Amber Glass leaf banner" | og-image.png (1200×630, repo root — served for all pages) | Generated — Recraft V4.1 standard via Higgsfield (Amber Glass leaf off-center left on deep green, room for title text right). Chosen by Damian over an evaporator-night photo candidate (that render kept in Higgsfield gallery — good seasonal/social material). Cover-cropped to og ratio via browser canvas. Disclosed. |
